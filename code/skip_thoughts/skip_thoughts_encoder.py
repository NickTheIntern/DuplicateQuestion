...
class SkipThoughtsEncoder(object):
  ...
  def _preprocess(self, data, use_eos):
    ...
    for item in data:
      ...
      if(len(tokenized) <= 0):
        print("error empty sentence :",item)
      if use_eos:
  ...
  def encode(self,
    ...
    """
    (data1, data2) = data
    data1 = self._preprocess(data1, use_eos)
    data2 = self._preprocess(data2, use_eos)
    features = []

    batch_indices = np.arange(0, len(data1), batch_size)
    for batch, start_index in enumerate(batch_indices):
      if verbose:
        tf.logging.info("Batch %d / %d.", batch, len(batch_indices))

      embeddings1, mask1 = _batch_and_pad(
          data1[start_index:start_index + batch_size])
      embeddings2, mask2 = _batch_and_pad(
          data2[start_index:start_index + batch_size])
      feed_dict = {
          "encode_emb1:0": embeddings1,
          "encode_emb2:0": embeddings2,
          "encode_mask1:0": mask1,
          "encode_mask2:0": mask2
      }
      #features.extend(
      #    sess.run("fc1/feature:0", feed_dict=feed_dict))
      temp = sess.run(["encoder/thought_vectors1:0","encoder/thought_vectors2:0"], feed_dict=feed_dict)
      features.extend(np.concatenate((temp[0],temp[1]), axis = 1)) 

    if use_norm:
      features = [v / np.linalg.norm(v) for v in features]
