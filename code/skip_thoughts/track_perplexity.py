...
def evaluate_model(sess, losses, accuracy, num_batches, global_step,
                   summary_writer, summary_op,model_path):
  ...
  for i in xrange(num_batches):
    batch_losses, batch_accuracy = sess.run([losses, accuracy])
    sum_losses += np.sum(batch_losses)
    sum_accuracies += batch_accuracy[0]
    if not i % 100:
      tf.logging.info("Computed losses for %d of %d batches.", i + 1,
                      num_batches)
  eval_time = time.time() - start_time

  average_loss = sum_losses/num_batches/128
  average_accuracy = sum_accuracies/num_batches

  tf.logging.info("average loss = %f (%.2f sec)", average_loss, eval_time)
  tf.logging.info("average accuracy = %f (%.2f sec)", average_accuracy, eval_time)
  global min_loss
  if(average_loss < min_loss):
     min_loss = average_loss
     call(['cp', model_path+".index", "/home/ubuntu/code/A_skip_thoughts_2/skip_thoughts/model/backup"])
     call(['cp', model_path+".meta", "/home/ubuntu/code/A_skip_thoughts_2/skip_thoughts/model/backup"])
     call(['cp', model_path+".data-00000-of-00001", "/home/ubuntu/code/A_skip_thoughts_2/skip_thoughts/model/backup"])
     tf.logging.info("saved model to backup")

  # Log perplexity to the SummaryWriter  
  summary = tf.Summary()
  value = summary.value.add()
  value.simple_value = average_loss
  value.tag = "avg_loss"
  value = summary.value.add()
  value.simple_value = average_accuracy
  value.tag = "avg_acc"
...
def run_once(model, losses, accuracy, saver, summary_writer, summary_op):
  ...
  with tf.Session() as sess:
    ...
    try:
      evaluate_model(sess, losses, accuracy, num_eval_batches, global_step,
                     summary_writer, summary_op, model_path)
...
def main(unused_argv):
  ...
  with g.as_default():
    ...
    while True:
      ...
      run_once(model, losses, accuracy, saver, summary_writer, summary_op)
